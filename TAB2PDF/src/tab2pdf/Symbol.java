...
public class Symbol {
	...
	public void createTextCenteredAtPosition(PdfContentByte canvas,
			String text, float x, float y, float size)
			throws DocumentException, IOException {
		canvas.setFontAndSize(BaseFont.createFont(BaseFont.HELVETICA,
	...
	public void createHLineAtPosition(PdfContentByte canvas, float x, float y,
			float length) {
		canvas.setLineWidth(0.5f);
	...
	public void createVLineAtPosition(PdfContentByte canvas, float x, float y,
			float length) {
		canvas.setLineWidth(0.5f);
		canvas.moveTo(x, SIZEY - y);
	...
	public void createCircle(PdfContentByte canvas, float x, float y,
			float length) {
	...
	public void createRDoubleBar(PdfContentByte canvas, float x, float y,
			float length) {
	...
	public void createLDoubleBar(PdfContentByte canvas, float x, float y,
			float length) {
		canvas.setLineWidth(1.8f);
		canvas.moveTo(x, SIZEY - y);
		canvas.lineTo(x, SIZEY - (y + length));
	...
	public void createS(PdfContentByte canvas, float x, float y, float length) {
		...
		canvas.addTemplate(diagonal, scaleX, tiltX, scaleY, tiltY, x
				- (length / 2), SIZEY - y);
	...
	public void createStar(PdfContentByte canvas, float x, float y,
			float length, boolean nextCharBar) {
		// PdfContentByte canvas = writer.getDirectContent();
		if (nextCharBar) {
	...
	public void createDiamond(PdfContentByte canvas, float x, float y,
			float size) {
		canvas.setLineWidth(0.5f);
		canvas.moveTo(x, SIZEY - y);
	...
	public void createArc(PdfContentByte canvas, float bottom, float top,
			float leftend, float rightend, float size) {
		canvas.arc(leftend - size, SIZEY - bottom, leftend + size, SIZEY - top,
				0, 180);
	}
}