...
public class drawOutput {
	...
	public void createPdf(String dest) throws DocumentException, IOException {
		 ...
		 */
		drawSymbol.createTextCenteredAtPosition(canvas, a.title, SIZEX / 2,
				LINEY * 1.5f, 24);
		drawSymbol.createTextCenteredAtPosition(canvas, a.subtitle, SIZEX / 2,
				LINEY * 2f, 12);
		...
		// for(int i = 0; i < 2; i++) // i = bar #
		{
			...
			for (int j = 0; j < a.input.get(i).size(); j++) // j = line number
															// in bar
			{
				...
				drawSymbol.createHLineAtPosition(canvas, 0, currY + j * SEGY,
						BEGINX);

				/*
				 * check size of current bar and test if it will fit within the
				 * page width when drawn if does not fit then continue in the if
				 * statement to end the current line with a vertical line, then
				 * start on a new line
				 */
				if (a.input.get(i).get(j).length() * a.spacing + currX > 570) {
					...
					for (int k = 0; k < 6; k++) {
						drawSymbol.createHLineAtPosition(canvas, currX, currY
								+ k * SEGY, SIZEX - currX);
					}
					cX = BEGINX;
					currX = cX;
				...
				List<Integer> diamondIndex = new ArrayList<Integer>(); // stores
																		// start
																		// and
																		// end
																		// indexes
																		// of
																		// diamonds
				int diamondIndexCount = 0; // counts number of diamonds in a
											// line

				while (matcher.find()) // stores indexes of diamond start and
				...
				for (int l = 0; l < tempString.length(); l++) {
					// realign string due to bug in parser, occurs on parsed
					// lines with vertical bars for repeat n times
					// ex. |4, the 4 gets parsed into the line making it 1
					// length longer than the rest in the segment
					if (l == 0
							&& j == 0
							&& a.input.get(i).get(j).substring(l, l + 1)
									.matches("[0-9]")) {
						tempString = tempString.substring(1);
					}

					// replaces blank space with a dash
					if (a.input.get(i).get(j).substring(l, l + 1)
							.matches("\\s")) {
					...
					if (hasDiamond == true
							&& diamondIndexCount < diamondIndex.size()) {
						if (l == diamondIndex.get(diamondIndexCount)) {
							System.out.println("Testing diamond");

							// get number value
							String diamondnum = tempString
									.substring(
											diamondIndex.get(diamondIndexCount) + 1,
											diamondIndex
													.get(diamondIndexCount + 1) - 1);
							System.out.println(diamondnum);
							int difference = (diamondIndex
									.get(diamondIndexCount + 1) - diamondIndex
									.get(diamondIndexCount));
							drawSymbol.createTextCenteredAtPosition(canvas,
									diamondnum, currX + (l + 1f) * a.spacing,
									currY + j * SEGY + 0.5f * a.spacing, 8);
							drawSymbol.createDiamond(canvas, currX
									+ (l + difference - 1) * a.spacing, currY
									+ j * SEGY - 0.5f * a.spacing, 2);
							// drawSymbol.createHLineAtPosition(canvas, currX +
							// (l + difference) * a.spacing, currY + j * SEGY,
							// a.spacing);

							l = l + difference; // advances character counter
												// past the diamond symbol
							diamondIndexCount = diamondIndexCount + 2;
						}
					}

					if (tempString.charAt(l) == '-') {
						drawSymbol.createHLineAtPosition(canvas, currX + l
								* a.spacing, currY + j * SEGY, a.spacing);
					} else if (tempString.charAt(l) == '*') {
						if (l == 0) {
							drawSymbol.createHLineAtPosition(canvas, currX + l
									* a.spacing, currY + j * SEGY, a.spacing);
							drawSymbol.createCircle(canvas, currX + a.spacing,
									currY + j * SEGY, a.spacing);
						} else {
							drawSymbol.createCircle(canvas, (currX + a.spacing
									* (l - 1)), currY + j * SEGY, a.spacing);
							drawSymbol.createHLineAtPosition(canvas, currX + l
									* a.spacing, currY + j * SEGY, a.spacing);
						}
					} else if (tempString.charAt(l) == 's') {
						drawSymbol.createS(canvas, currX + l * a.spacing, currY
								+ j * SEGY, a.spacing);
					} else if (tempString.charAt(l) == 'h'
							|| tempString.charAt(l) == 'p') {
						drawSymbol.createArc(canvas,
								currY + (j - 0.75f) * SEGY, currY + (j - 0.25f)
										* SEGY,
								currX + (l + 0.50f) * a.spacing, currX
										+ (l + 5) * a.spacing, a.spacing);
						drawSymbol.createHLineAtPosition(canvas, currX + l
								* a.spacing, currY + j * SEGY, a.spacing);
						drawSymbol.createTextCenteredAtPosition(canvas, ""
								+ tempString.charAt(l), currX + (l + 0.5f)
								* a.spacing, currY + (j - 1.25f) * SEGY + 0.5f
								* a.spacing, 5);
					} else {
						drawSymbol.createTextCenteredAtPosition(canvas, ""
								+ tempString.charAt(l), currX + (l + 0.5f)
								* a.spacing, currY + j * SEGY + 0.5f
								* a.spacing, 8);
					}
				}
		...
		for (int k = 0; k < 6; k++) {
			drawSymbol.createHLineAtPosition(canvas, currX, currY + k * SEGY,
					SIZEX - currX);
		}
		document.close();
	}

	public void DrawVerticalBar(int i, int j, PdfContentByte canvas,
			float currX, float currY) throws DocumentException, IOException {
		...
		if (vLines.equals("|||")) {
			...
			drawSymbol.createVLineAtPosition(canvas, currX + a.spacing, currY,
					LINEY);
			drawSymbol.createVLineAtPosition(canvas, currX + 2 * a.spacing,
					currY, LINEY);
			...
			drawSymbol.createTextCenteredAtPosition(canvas,
					"Repeat " + vLines.charAt(1) + " times", currX - 5
							* a.spacing, currY - 1.5f * a.spacing, 8);
		} else if (vLines.equals("|")) {
			drawSymbol.createVLineAtPosition(canvas, currX, currY, LINEY);
		}
		vCounter++;
