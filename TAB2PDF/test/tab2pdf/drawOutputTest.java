...
public class drawOutputTest
{
	...
	public void testCheckNewLine() throws FileNotFoundException, DocumentException
	{
		drawOutput d = new drawOutput(p1);
		PdfContentByte canvas = d.initPDF(outURL);
		d.resetXY();
	...
	public void testDrawVerticalBar() throws DocumentException, IOException
	{	//t3.txt
		...
		float beforeTestX = d.getCurrX();
		int vCounter = 0;
		int new_vCounter = d.DrawVerticalBar(0, 0, canvas, d.getCurrX(), d.getCurrY(), vCounter);
	...
	public void testDrawSegment() throws DocumentException, IOException
	{
		...
		assertEquals(beforeTestY, d.getCurrY(), 0.0001);
		assertEquals(beforeTestX + p3.input.get(i).get(0).length()*p3.spacing, d.getCurrX(), 0.0001);
